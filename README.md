...
安装后，Jarvis 在不同平台走各自原生入口：

- **Claude Code**：通过 hooks 在会话启动时注入 Core
- **Reasonix**：通过 `reasonix.toml` 的 `system_prompt_file` + `skills.paths` 触发 Jarvis bootstrap
- **Codex**：通过 `AGENTS.md` + `~/.codex/skills/` 触发 Jarvis bootstrap
...
# 预览安装操作
jarvis install --target reasonix --dry-run
```

平台接法：

- `claude`：安装全局 hook + skill 软链；重复安装会自动重连旧 Jarvis 软链，并替换旧 Jarvis hook 配置。
- `reasonix`：写入 `~/.reasonix/config.toml` 的 `system_prompt_file` / `skills.paths`，并清理旧 `~/.reasonix/settings.json` 中残留的 Jarvis hooks。
- `codex`：安装 `~/.codex/skills/`，项目侧通过 `AGENTS.md` 完成 bootstrap。

注意：Reasonix 的项目本地 `reasonix.toml` 优先级高于 `~/.reasonix/config.toml`。因此 `jarvis init` 还会在项目根生成/合并：

- `reasonix.toml`
- `REASONIX.md`

这样同一个项目在 Reasonix 中不会因为本地配置覆盖全局 bridge 而失效。
...
Core 规则升级分两步：先升级框架本体和全局适配器，再对每个已有项目运行 `jarvis init --sync <project>`。只运行 `upgrade` / `install` 不会刷新项目里已经嵌入的 `AGENTS.md` / `REASONIX.md` runtime block。

`--sync` 会更新 `jarvis.yaml` 的版本、`jarvis_home` 和缺失的 `platform`，刷新 `AGENTS.md` / `REASONIX.md` / `reasonix.toml`，并移除项目级 `.claude/settings.json` 里旧的 Jarvis hooks，避免全局 hook 与项目 hook 重复触发。如果已经通过 npm / pipx 完成框架升级，可以从 `jarvis install --target all` 开始执行。
...
jarvis init ~/my-project --platform reasonix
jarvis init ~/my-project --platform codex

# 同一项目三端通用
jarvis init ~/my-project --platform all
...
- `REASONIX.md`：Reasonix 的专用 runtime prompt
