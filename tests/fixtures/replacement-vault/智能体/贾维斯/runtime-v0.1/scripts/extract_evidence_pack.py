...
def build_item(
    *,
    ...
    target_path = target_path or (f"知识库/术语/{title}.md" if knowledge_layer == "L1" else "")
    return {
        "id": item_id,
        "group": group,
        "title": title,
...
def extract_relation_items(path: Path, heading: str, items: list[str], topic_name: str, topic_dir: Path, counters: Counter[str]) -> list[dict]:
    output: list[dict] = []
    target_path = path.relative_to(topic_dir.parents[3]).as_posix() if "业务/" in path.as_posix() else ""
    for entry in items:
        output.append(
            build_item(
...
def extract_decision_items(path: Path, heading: str, items: list[str], topic_name: str, topic_dir: Path, counters: Counter[str]) -> list[dict]:
    output: list[dict] = []
    target_path = path.relative_to(topic_dir.parents[3]).as_posix() if "业务/" in path.as_posix() else ""
    for entry in items:
        output.append(
...
def extract_risk_items(path: Path, heading: str, items: list[str], topic_name: str, topic_dir: Path, counters: Counter[str]) -> list[dict]:
    output: list[dict] = []
    target_path = path.relative_to(topic_dir.parents[3]).as_posix() if "业务/" in path.as_posix() else ""
    for entry in items:
...
def infer_items_from_file(path: Path, topic_name: str, topic_dir: Path, counters: Counter[str]) -> list[dict]:
    text = read_text(path)
    ...
    for heading, body in sections.items():
        ...
        if any(keyword in heading for keyword in DEFINITION_HEADINGS):
            ...
            output.extend(extract_relation_items(path, heading, items, topic_name, topic_dir, counters))
        elif any(keyword in heading for keyword in DECISION_HEADINGS):
            output.extend(extract_decision_items(path, heading, items, topic_name, topic_dir, counters))
        elif any(keyword in heading for keyword in RISK_HEADINGS):
            output.extend(extract_risk_items(path, heading, items, topic_name, topic_dir, counters))
    return output
...
def main() -> int:
    ...
    for path in sorted(used_paths):
        items.extend(infer_items_from_file(path, topic_name, topic_dir, counters))
