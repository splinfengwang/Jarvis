...
confirmation_rules:
- 关闭只同步状态，不自动萃取、不自动入库。
- 关闭前必须处理最终关联会话：① 扫描中间会话：`locate_session_jsonl.py --tool claude-code --cwd <项目路径> --date <上次记录日期>` 找到遗漏的中间会话 → 补入关联会话表。② 当前会话：`--latest` 追加。定位不到 → 标 `待确认 — tool=claude-code, date=now`。
